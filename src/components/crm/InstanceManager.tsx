...
import { FacebookInstanceConnect } from './FacebookInstanceConnect'
...
export function InstanceManager() {
  ...
  const [formProvider, setFormProvider] = useState<'baileys' | 'cloud_api' | 'facebook'>('baileys')
      ...
      {showForm && (
        <div className="mb-4 bg-slate-900 border border-slate-700 rounded-xl p-4 space-y-3">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button
              onClick={() => setFormProvider('facebook')}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg border text-sm font-medium transition ${
                formProvider === 'facebook'
          ...
          {/* Fluxo Facebook */}
          {formProvider === 'facebook' && (
            <FacebookInstanceConnect
              onCreated={async () => { resetForm(); await load() }}
              onCancel={resetForm}
            />
          )}

          {/* Nome da instância — oculto no fluxo Facebook (gerenciado pelo componente) */}
          {formProvider !== 'facebook' && (
            <input
              value={formName}
